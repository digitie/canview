...
`STM32G474CEU6`용 C99 boot/fault bench firmware foundation이다. safe-state 뒤 bounded HSE/PLL·IWDG·TIM2/SysTick과 cooperative health scheduler를 실행하고 reset/build/stack 진단 record를 준비한다. T-103의 3채널 FDCAN capture-only module과 CMSIS adapter source도 target에 포함하지만, 기본 `app/main.c`는 실제 peripheral/task wiring을 시작하지 않는다. UART transport·SPORT·OTA는 활성화하지 않는다. [core 구조·API 소유권·ISR·시험·미실행 gate](docs/core-bench.md)와 [FDCAN capture 계약](docs/fdcan-capture.md)이 현재 구현 계약이다. 기존 자동 SPORT prototype은 별도 host 회귀에서만 유지한다.
...
- UART 4 Mbps transport wiring은 후속 task에서 추가한다. T-103 FDCAN adapter는 검증된 profile에서만 capture mode를 시작할 수 있고, 기본 app은 이를 호출하지 않는다. 모든 CAN TX request GPIO는 high로 유지되며 외부 gate/PHY receive 전환은 실제 G2 시험 전까지 안전 상태다.
