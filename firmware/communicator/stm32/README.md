...
- build mode는 compile-time `CAPTURE_ONLY` 하나로 고정한다. `CANVIEW_STM_ENABLE_BENCH_TX`, `CANVIEW_STM_ENABLE_VEHICLE_TX`와 mode override는 거부하며 control capability/TX permit은 0/false다.
...
- reset reason/build digest/profile/stack 상태를 pointer 없는 40-byte little-endian diagnostic record로 encode한다. 실제 UART 송신과 최종 UART ABI 연결은 T-104 범위다.
