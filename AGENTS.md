...
- 요구사항을 만족하는 가장 단순한 구현을 우선한다. 기존 SDK·표준 기능을 먼저 재사용하고, 현재 task에서 실제로 쓰지 않는 범용 프레임워크·확장 기능·추상화는 미리 만들지 않는다.
- 새 계층·의존성·파일 형식이 필요하면 더 단순한 대안으로 충족하지 못하는 요구를 먼저 설명한다. 단순화는 필수 기능·안전 경계·검증 gate를 생략하는 근거가 아니다.
