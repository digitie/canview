...
def stm32(c):
    s = c.sheet('04_stm32', 'STM32G474CEU6 / UFQFPN48. HSE 16MHz. USART2 4Mbps RTS/CTS. SWD only; USB pins are CAN1.')
    ...
    c.from_library(s,'U10','STM32G474CEU6','Package_DFN_QFN:QFN-48-1EP_7x7mm_P0.5mm_EP5.6x5.6mm','MCU_ST_STM32G4','STM32G474CEUx',nets,source='stm32g474: UFQFPN48 pinout, AF9/AF11/AF7; stm32g4-hardware')
    ...
    c.ic(s,'Y1','ABM8-16.000MHZ-B2-T','Crystal:Crystal_SMD_3225-4Pin_3.2x2.5mm', ['XI','GND','XO','GND'],{'1':'HSE_IN','2':'GND','3':'HSE_DRV','4':'GND'},spec='16MHz; CL18pF; ±20ppm; ±50ppm temperature; ESR80ohm maximum; verify drive power',source='abracon-abm8')
    c.r(s,'0','HSE_OUT','HSE_DRV',note='HSE drive-limiting series R footprint; tune per AN2867 before release')
    ...
    c.ic(s,'J10','SWD 2x5 1.27mm','Connector_PinHeader_1.27mm:PinHeader_2x05_P1.27mm_Vertical',
         ['VTref_OUT','SWDIO','GND','SWCLK','GND','SWO_NC','KEY_NC','NC','GND','NRST'],{'1':'3V3','2':'SWDIO','3':'GND','4':'SWCLK','5':'GND','9':'GND','10':'SYS_RESET_N'},spec='Keyed shrouded matching cable required; VTref sense only, no external power',note='Use pin1 marking; SWO not routed')
...
def esp32(c, bridge=False):
    s = c.sheet('06_esp32' if not bridge else '02_esp32', 'ESP32-S3-MINI-1-N4R2. GPIO26 reserved for PSRAM. Keep antenna clearance. USB native serial/JTAG.')
    from kicad_model import library_pins
    pins=library_pins('RF_Module','ESP32-S3-MINI-1')
    ...
    if not bridge:
        nets.update({'19':'ESP_RTS_SRC','20':'STM_RTS','21':'ESP_TX_SRC','22':'STM_TX',
                     ...
                     '12':'BARO_SDA','13':'BARO_SCL','14':'IMU_SCLK','15':'IMU_MOSI',
                     '16':'IMU_MISO','17':'IMU_CS_N','18':'IMU_DRDY','25':'IMU_RESET_N',
                     '27':'GPS_PWR_REQ','28':'USB_SERVICE_SENSE'})
    else:
        nets.update({'8':'PAIR_BUTTON_N','9':'STATUS_LED','12':'SD_CS_N','13':'SD_MISO','14':'SD_SCLK','15':'SD_MOSI'})
    c.from_library(s,'U11' if not bridge else 'U1','ESP32-S3-MINI-1-N4R2','RF_Module:ESP32-S2-MINI-1','RF_Module','ESP32-S3-MINI-1',nets,source='esp32s3-mini: pin table, peripheral schematic and land pattern',spec='4MB flash + 2MB PSRAM; 3.0..3.6V; -40..85C')
    ...
    if bridge:
        ...
        c.two(s,'D','BLUE','LED_A','GND',fp='LED_SMD:LED_0603_1608Metric',mpn='GEN-LED-0603-BLUE',spec='Blue LED; If<=1mA; Vf<=3.2V')
...
def can_ports(c):
    ...
    c.r(s,'10k','PHY3V3','CAN3_TXD');c.r(s,'10k','CAN3_EN','GND')
...
def tx_gate(c):
    ...
    c.ic(s,'U25','SN74LVC2G125DCUR',VSS8,names,{'1':'TX_OE_N','2':'CAN3_TX_REQ','3':'CAN3_EN','4':'GND','5':'FT_EN_REQ','6':'CAN3_TXD','7':'FT_RX_OE_N','8':'PHY3V3'},types=types,source='sn74lvc2g125',spec='CAN3 TX and RX-mode enable are independently gated')
    c.r(s,'10k','PHY3V3','TX_OE_N');c.r(s,'10k','PHY3V3','FT_RX_OE_N')
    for _ in range(2):c.c(s,'100n','PHY3V3',size='0402')
    ...
    inv(c,s,'U27','AUTO_GOOD','FT_RX_OE_N')
    c.c(s,'100n','PHY3V3',size='0402')
    s=c.sheet('12_watchdog_latch','50ms watchdog clears ARM latch asynchronously. WDO recovery alone CANNOT re-enable TX. ARM edge must be renewed.')
    ...
    c.c(s,'1n','WDT_C',dielectric='C0G',tolerance='5%',size='0402');c.c(s,'100n','PHY3V3',size='0402');c.r(s,'10k','PHY3V3','WD_OK_N')
    gate3(c,s,'U31','WD_OK_N','SYS_RESET_N','AUTO_GOOD','ARM_CLEAR_N')
    ...
    gate3(c,s,'U33','ARM_LATCH','TX_ARM','SYS_RESET_N','TX_PERMIT')
