...
구조적 결정은 파일당 하나의 ADR로 docs/adr/ 아래에 기록한다. 이 문서는 목록과 현재 상태만 제공하며 상세 근거는 각 ADR가 정본이다. 다음 후보 번호는 ADR-010이다.
...
| ADR-009 | accepted | SDK 재사용을 위한 OTA image 정렬 | [009-ota-native-image-alignment.md](adr/009-ota-native-image-alignment.md) |
