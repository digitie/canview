# 기록: R1 이전 Communicator 설계 입력

이 문서는 과거 검토 이력이며 현재 회로 지시가 아니다. USB data-only, PGOOD 직결, 과거 TCAN 핀과 미정 gate 내용을 구현에 사용하지 않는다. 현재 정본은 [R1 상세 회로](../r1/README.md)다.
...
당시 기계 판독용 제안은 [이전 pin proposal](../../../hardware/communicator/history/pinmap-pre-r1.csv)에 보존한다. 현재 정본이 아니며 오류를 포함하므로 구현에 사용하지 않는다.
