...
| reset | PG10-NRST/7 | 반드시 NRST 기능 보존, GPIO로 전환 금지 |
...
|---|---|---|
| STM UART RTS | 15/19 | STM PA0 CTS |
| STM UART CTS | 16/20 | STM PA1 RTS |
| STM UART TX | 17/21 | STM PA3 RX |
| STM UART RX | 18/22 | STM PA2 TX |
| native USB D−/D+ | 19/23,20/24 | ESD/22Ω 뒤 connector |
| GPS direct TX 선택 | 4/8 | DNP 링크. 기본 MTi TX와 동시 실장 금지 |
| GPS RX tap | 5/9 | GNSS TX buffered input, 기본 read-only |
| PPS | 6/10 | GNSS PPS buffered input, 측정 epoch 동기 |
| MTi SPI SCLK/MOSI/MISO/CS | 10/14,11/15,12/16,13/17 | mode3,≤2MHz, single owner |
| MTi DRDY | 14/18 | IRQ는 timestamp+task notification만 |
| MTi reset | 21/25 | **open-drain**, HIGH를 강제로 출력하지 않음 |
| GPS power 요청 | 47/27 | external100k PD, AUTO_GOOD/SYS_RESET과 hardware AND |
| mux source 상태 | 33/28 | USB_SERVICE_SENSE, rail-good 신호 아님 |
| BOOT / EN | 0/4, CHIP_PU/45 | BOOT release·SYS_RESET_N |

module pad3=3V3다. 전체 GND와 EP pad는 local footprint/pinmap에서 확인한다. 메모리용 핀을 GPIO처럼 회수하지 않는다. N4R2와 Bridge N8R2의 module pad를 복사해 쓰지 않는다.
...
1. BSP GPIO 초기값 PA4/5 HIGH,PA6/7 LOW를 먼저 latch에 쓰고 output mode를 설정한다. UART CTS/RTS는 외부 PU 덕분에 두 MCU 준비 전 flow stop이다.
