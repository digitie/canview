...
최초 OTA 변경은 회로 생성 입력과 KiCad 산출물에 적용했다. ESP-IDF 프로젝트, STM linker, 실제 부트로더·웹 서버·서명 키 provisioning은 아직 구현하지 않았다. 이 문서의 Flash 표는 후속 구현 계약이며 현재 CSV가 이미 OTA용이라는 뜻이 아니다. 최초 검토는 사용자의 단일 MD 요청에 따라 리뷰까지 본문에 기록했다. 이후 전체 계획 재점검 요청으로 구현 작업은 [task 요약](../tasks.md)의 T-007/T-107/T-108/T-204/T-205/T-306/T-507/T-508로 분리했고, 본문 §14의 과거 독립 리뷰 원문은 그대로 보존한다.
